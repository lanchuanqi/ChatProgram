...
/* Define the upper limitaion for capacity*/
 #define CHANNEL_NUM 108
 #define USER_NUM 108
 #define ID_NUM 100000
...
/*This is a structure for server to store various incoming socket connetion*/
struct socket_clients {
        int socket;
        int userChannelNum;
        char userChannel[CHANNEL_MAX];
        char userName[USERNAME_MAX];
        struct channel_info joinChannels[CHANNEL_NUM];
...
struct text_list {
        ...
        int txt_nchannels;
        struct channel_info txt_channels[CHANNEL_MAX]; // May actually be more than 0
} packed;
...
struct text_who {
        ...
        struct user_info txt_users[CHANNEL_MAX]; // May actually be more than 0
} packed;
...
struct channel_tree{
        struct channel_info channelName;
        char *serverInf[SERVER_MAX];
        int portNum[SERVER_MAX];
        int serverNum;
        int timerMark[SERVER_MAX];
        //time_t timer;
} packed;
